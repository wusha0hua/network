...
char baidu_mac[6]={0x54,0x75,0x95,0xb9,0x45,0x17};
//char baidu_ip[4]={0xdc,0xb5,0x26,0x94};
char baidu_ip[]="220.181.38.148";
...
void EtherHeaderCreate(unsigned char *buf,DeviceInfo *dvif)
{
	...
	memcpy(eth->ether_dhost,baidu_mac,6);
...
void IPPacketCreate(unsigned char *buf,DeviceInfo *dvif)
{
	...
	ip->ihl=5;
	ip->tos=0;
	ip->tot_len=htons(20+sizeof(struct icmp));
	ip->id=1;
	ip->frag_off=0;
	ip->ttl=255;
	...
	//ip->daddr=(baidu_ip[0])|(baidu_ip[1]<<8)|(baidu_ip[2]<<16)|(baidu_ip[3]<<24);
	ip->daddr=inet_addr(baidu_ip);

	ip->check=(Check((unsigned short*)ip,10));

}

void IcmpPacketCreate(unsigned char *buf)
{
	struct icmp *icmp;
	icmp=buf;
	...
	icmp->icmp_code=0;
	icmp->icmp_cksum=0;
	icmp->icmp_hun.ih_idseq.icd_id=htons(16);
	icmp->icmp_hun.ih_idseq.icd_seq=htons(16);
...
void IcmpSend(unsigned char *packet,DeviceInfo *dvif)
{
	...
	memcpy(sll.sll_addr,baidu_mac,6);
	...
	while(1)
	{
		int r=sendto(fd,packet,sizeof(struct iphdr)+sizeof(struct ether_header)+sizeof(struct icmp),0,(struct sockaddr *)&sll,sizeof(struct sockaddr_ll));
		if(r<0)
		{
			puts("error");
		}
...
int main()
{
	...
	IcmpPacketCreate(packet+sizeof(struct ether_header)+sizeof(struct iphdr));

	for(int i=0;i<sizeof(struct ether_header)+sizeof(struct iphdr)+sizeof(struct icmp);i++)
	{
		if(i%16==0)
		{
			puts("");
